<template>
	<div class="work-inspect-box row-box">
		<div class="repair-tab">
			<span v-for="(li,i) in tabList" :class="{active:activeTab == i}" :key="i" @click="tabClick(i)">{{li}}</span>
		</div>
...
<script>
	...
	export default{
		...
		data(){
			...
			return{
				...
				wxData:{
					...
					tablec:null,//表格数据
					inspectionData:{
						arr:null,result:null,phone:null,query:null,isSave:null,workfootData:null,aProcedureTxt:null
					},//当前页的整理数据
				},
				swiperOption:{
					initialSlide:0,
		...
		methods:{
			...
			getcheckdetaildata(){//获取保养任务数据
				this.byData.show = false;
				this.$store.commit('showLoading')
				this.$axios.get(this.$root.URL+'eom/api/turn/MaintainDetail/'+this.$route.query.id).then((res) => {
			...
			dataHandle(data){//保养任务整理数据
				let dd = this.byData.obj;
				let d = this.byData.obj.results[this.byData.handle.page - 1];
...
// 					{name:'结束时间',value:d.endTime || ''},
			...
			dataHandle1(){//维修任务整理数据
				let dd = this.wxData.obj;
				let d = this.wxData.obj.results[this.wxData.handle.page - 1];
				...
				this.wxData.inspectionData.arr = [
					...
					{name:'开始时间',value:(d.startDateMb || '未开始')+' '+(d.startTimeMb || '')},
					{name:'结束时间',value:(d.endDateMb || '未结束')+' '+(d.endTimeMb || '')},
			...
			sureSubmit(){//保养任务确认提交
				...
				this.$store.commit('showLoading');
				this.$axios.put(this.$root.URL+'eom/api/turn/submitMaintain/'+dd.id,{
					result:dd.result,
			...
			pRepairdetail(){//获取维修工单
				...
				this.$store.commit('showLoading');
				this.$axios.get(this.$root.URL+'eom/api/turn/RepairDetail/'+this.$route.query.id)
				.then(res => {
					this.$store.commit('hideLoading');
					let data = res.data;
					if(!data.result){
			...
			pRepairStart(){//开始维修
				...
				this.$store.commit('showLoading');
				this.$axios.put(this.$root.URL+'eom/api/turn/startRepair/'+work.id,{
			...
			pRepairSubmit(){//结束维修
				let work = this.wxData.inspectionData.workfootData;
				let list = this.wxData.inspectionData;
				//work.aProcedureTxt = list.aProcedureTxt.value;
				this.$store.commit('showLoading');
				this.$axios.put(this.$root.URL+'eom/api/turn/endRepair/'+work.id,{
		...
		beforeRouteEnter(to, from, next){
			next(vm => {
				if(from.meta.index < to.meta.index){
					if(to.query.msg){
						vm.headData[1].html = vm.$route.query.name+'（轮保）';
						...
						vm.headData[1].html = vm.$route.query.equName+'（轮保）';
						vm.byData.handle.page = 1;
						vm.wxData.handle.page = 1;
					}
