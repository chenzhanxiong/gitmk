	...
	export default{
		...
		methods:{
			...
			getcheckdetaildata(){
				...
				this.$axios.get(this.$root.URL+'eom/api/checkRepair/MaintainDetail/'+this.$route.query.id).then((res) => {
			...
			sureSubmit(){//确认提交
				...
				this.$store.commit('showLoading');
				this.$axios.put(this.$root.URL+'eom/api/checkRepair/submitMaintain/'+dd.id,{
					result:dd.result,
			...
			pRepairdetail(){//获取维修工单
				...
				this.$store.commit('showLoading');
				this.$axios.get(this.$root.URL+'eom/api/checkRepair/RepairDetail/'+this.$route.query.id)
				.then(res => {
					this.$store.commit('hideLoading');
					let data = res.data;
					if(!data.result){
			...
			pRepairStart(){//开始维修
				...
				this.$store.commit('showLoading');
				this.$axios.put(this.$root.URL+'eom/api/checkRepair/startRepair/'+work.id,{
			...
			pRepairSubmit(){//结束维修
				let work = this.wxData.inspectionData.workfootData;
				this.$store.commit('showLoading');
				this.$axios.put(this.$root.URL+'eom/api/checkRepair/endRepair/'+work.id,{
		...
		beforeRouteEnter(to, from, next){
			next(vm => {
				if(from.meta.index < to.meta.index){
					if(to.query.msg){
						vm.headData[1].html = vm.$route.query.name+'（检修）';
						vm.byData.obj = null;
						vm.wxData.obj = null;
