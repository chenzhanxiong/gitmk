				...
				operation:[//点击列表出现按钮数据
					{
						...
						show:true,
						event:(list) => {
							this.$MessageBox.confirm('确认开始周保？').then(action => {
								this.putStart();
							})
						}
					},
...
// 					},
					{
						...
						show:true,
						event:(list) => {
							this.$router.push({path:'/detailLun5',query:{id:list.id,name:list.name}});
						}
					},
				],
				screenData:{//查询条件
		...
		methods:{
			getlistData(type){//获取点检工单列表
				...
				this.$axios.get(this.$root.URL+'eom/api/week/list',{
					params:{
						params:{
							query:{
			...
			putStart(){//开始
				this.$store.commit('showLoading')
				this.$axios.put(this.$root.URL+'eom/api/week/start/'+this.$store.state.worklistData.worklisteQuKeyItem.id).then((res) => {
					this.$store.commit('hideLoading')
					let d = res.data;
					if(!d.result){
			...
			putDelay(data){//提交延期
				...
				this.$store.commit('showLoading')
				this.$axios.put(this.$root.URL+'eom/api/week/delay/'+this.$store.state.worklistData.worklisteQuKeyItem.id,{
					delayDate:data
				}).then((res) => {
					this.$store.commit('hideLoading')
					let d = res.data;
			...
			submitAll(list){//提交完成
				this.$store.commit('showLoading')
				this.$axios.put(this.$root.URL+'eom/api/week/submitAll/'+this.$store.state.worklistData.worklisteQuKeyItem.id).then((res) => {
					this.$store.commit('hideLoading')
					let d = res.data;
					if(!d.result){
		...
		activated(){//默认执行
			if(!this.$route.meta.isUseCache || this.listdata.length <= 0){
				this.pageIndex = 1;
				this.$store.state.repairData.title = '周保工单';
				//this.getScreen();
				this.screenarr();
