<template>
	<div class="workInspectlun-box row-box">
		...
		<div v-if="show && datalist.length > 0" class="workinspect-list">
			<h3 class="workinspect-title">周保设备</h3>
	...
	export default{
			...
			return{
				...
				operationc:[
					...
					},
					{
						...
						show:true,
						event:(list) => {
							this.$router.push({path:'/workHandle5',query:{...list}});
						}
					},
				],
				headData:[//title数据
		...
		methods:{
			getPeriodFindDetail(){//获取数据
				...
				this.$axios.get(this.$root.URL+'eom/api/week/detail/'+this.$route.query.id).then((res) => {
					this.show = true;
			...
			putStart(list){//开始
				this.$store.commit('showLoading')
				this.$axios.put(this.$root.URL+'eom/api/week/startChild/'+list.id).then((res) => {
					this.$store.commit('hideLoading')
					let d = res.data;
					if(!d.result){
			...
			endChild(list){//完成
				this.$store.commit('showLoading')
				this.$axios.put(this.$root.URL+'eom/api/week/endChild/'+list.id).then((res) => {
					this.$store.commit('hideLoading')
					let d = res.data;
					if(!d.result){
						...
						this.$MessageBox.confirm('发现<font color="red">'+ty[msg.ident]+'</font>第<font color="red">'+msg.num+'</font>项未保存<br/>是否跳转至该项？').then(() => {
							this.$router.push({path:'/workHandle5',query:{id:list.id,name:list.name,msg:msg}});
						})
					}
				})
			}
