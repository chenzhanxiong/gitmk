<template>
	<wroklistc :listdata="listdata" :query="$route.query" ref="child" :operationc="operation" @putDelay="putDelay" @getlistData="getlistData(true)"></wroklistc>
</template>
...
<script>
	...
	export default{
			...
			return{
				...
				operation:[
					...
					},
					{
						...
						show:true,
						event:(list) => {
							this.$router.push({path:'/workHandle1',query:{id:list.id,name:list.name}});
						}
					},
				],
				screenData:{
		...
		methods:{
			...
			getScreen(){//查询筛选条件
				let code = this.worklistData.worklistactiveItem.code;
				this.$axios.all([
			...
			putDelay(data){//提交延期
				...
				this.$store.commit('showLoading')
				this.$axios.put(this.$root.URL+'eom/api/check/delay/'+this.$store.state.worklistData.worklisteQuKeyItem.id,{
					dDate:data
				}).then((res) => {
					console.log(res)
					this.$store.commit('hideLoading')
			...
			submitAll(list){//提交全部
				this.$store.commit('showLoading')
				this.$axios.put(this.$root.URL+'eom/api/check/submitAll/'+this.$store.state.worklistData.worklisteQuKeyItem.id).then((res) => {
					this.$store.commit('hideLoading')
					let d = res.data;
					if(!d.result){
						...
						this.$MessageBox.confirm('发现第<font color="red">'+d.errorMsg+'</font>项未保存<br/>是否跳转至该项？').then(() => {
							this.$router.push({path:'/workHandle1',query:{id:list.id,name:list.name,page:d.errorMsg}});
						})
					}
				})
			},
		...
		activated(){
			if(!this.$route.meta.isUseCache || this.listdata.length <= 0){
				this.pageIndex = 1;
				this.$store.state.repairData.title = '点检工单';
				this.getScreen();
			}
			mui.back = function(){
